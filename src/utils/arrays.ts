import { navLinkType } from "./interface";

export const navLinks: navLinkType[] = [
  {
    url: "/",
    text: "My Dashboard",
    icon: `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M13 8H8V13H13V8ZM13 3H8V7H13V3ZM15 0H1C0.45 0 0 0.45 0 1V15C0 15.55 0.45 16 1 16H15C15.55 16 16 15.55 16 15V1C16 0.45 15.55 0 15 0ZM14 14H2V2H14V14ZM7 3H3V13H7V3Z" fill="#FAFBFF"/>
</svg>
`,
  },
  {
    url: "/user-management",
    text: "User Management",
    icon: `<svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M3.5 6H10.5C10.78 6 11 5.78 11 5.5C11 5.22 10.78 5 10.5 5H3.5C3.22 5 3 5.22 3 5.5C3 5.78 3.22 6 3.5 6ZM15 0H1C0.45 0 0 0.45 0 1V13C0 13.55 0.45 14 1 14H15C15.55 14 16 13.55 16 13V1C16 0.45 15.55 0 15 0ZM14 12H2V4H14V12ZM3.5 8H7.5C7.78 8 8 7.78 8 7.5C8 7.22 7.78 7 7.5 7H3.5C3.22 7 3 7.22 3 7.5C3 7.78 3.22 8 3.5 8ZM3.5 10H8.5C8.78 10 9 9.78 9 9.5C9 9.22 8.78 9 8.5 9H3.5C3.22 9 3 9.22 3 9.5C3 9.78 3.22 10 3.5 10Z" fill="#8F95B2"/>
</svg>
`,
  },
  {
    url: "/website-management",
    text: "Website Mgt",
    icon: `<svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M15 14H1C0.45 14 0 13.55 0 13V1C0 0.45 0.45 0 1 0H15C15.55 0 16 0.45 16 1V13C16 13.55 15.55 14 15 14ZM14 4H2V12H14V4ZM4 5C4.28 5 4.53 5.11 4.71 5.29L6.71 7.29C6.89 7.47 7 7.72 7 8C7 8.28 6.89 8.53 6.71 8.71L4.71 10.71C4.53 10.89 4.28 11 4 11C3.45 11 3 10.55 3 10C3 9.72 3.11 9.47 3.29 9.29L4.59 8L3.29 6.71C3.11 6.53 3 6.28 3 6C3 5.45 3.45 5 4 5ZM9 9H12C12.55 9 13 9.45 13 10C13 10.55 12.55 11 12 11H9C8.45 11 8 10.55 8 10C8 9.45 8.45 9 9 9Z" fill="#8F95B2"/>
</svg>
`,
  },
  {
    url: "/template-management",
    text: "Template Mgt.",
    icon: `<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M12.99 -0.0100098H0.98999C0.43999 -0.0100098 -0.0100098 0.43999 -0.0100098 0.98999C-0.0100098 1.26999 0.0999902 1.51999 0.27999 1.69999L4.98999 6.40999V13C4.98999 13.55 5.43999 14 5.98999 14C6.26999 14 6.51999 13.89 6.69999 13.71L8.69999 11.71C8.87999 11.53 8.98999 11.28 8.98999 11V6.40999L13.7 1.69999C13.88 1.51999 13.99 1.26999 13.99 0.98999C13.99 0.43999 13.54 -0.0100098 12.99 -0.0100098Z" fill="#8F95B2"/>
</svg>
`,
  },
  {
    url: "/statistics",
    text: "Statistics ",
    icon: `<svg width="16" height="13" viewBox="0 0 16 13" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M14 9H7.72L7.39 8H13C13.39 8 13.72 7.77 13.89 7.44H13.9L15.9 3.44H15.89C15.95 3.31 16 3.16 16 3C16 2.45 15.55 2 15 2H5.39L4.95 0.68H4.94C4.8 0.29 4.44 0 4 0H1C0.45 0 0 0.45 0 1C0 1.55 0.45 2 1 2H3.28L5.61 9H4C2.9 9 2 9.9 2 11C2 12.1 2.9 13 4 13C5.1 13 6 12.1 6 11H12C12 12.1 12.9 13 14 13C15.1 13 16 12.1 16 11C16 9.9 15.1 9 14 9ZM6.05 4H13.38L12.38 6H6.72L6.05 4Z" fill="#8F95B2"/>
</svg>
`,
  },
];
